...
export interface ChartProComponentProps
  extends Required<Omit<ChartProOptions, "container" | "periodBarTools">> {
...
const ChartProComponent: Component<ChartProComponentProps> = (props) => {
  ...
  let loading = false;
  let pendingCandleType: CandleType | undefined;
  ...
  const applyPeriodCandleType = (currentPeriod: Period) => {
    const isTimeLine =
      currentPeriod.text === "分时" || currentPeriod.timespan === "time";
    // @ts-expect-error candle type exists on styles
    const allowedTypes: CandleType[] = isTimeLine
      ? ["area"]
      : [
          "candle_solid",
          "candle_stroke",
    ...
    // @ts-expect-error candle type exists on styles
    const defaultType: CandleType = isTimeLine ? "area" : "candle_up_stroke";

    let currentType: CandleType | undefined;
    if (widget) {
      try {
        const currentStyles = widget.getStyles() as Styles;
        currentType = currentStyles?.candle?.type as CandleType | undefined;
      } catch (e) {
        currentType = undefined;
      }
    } else if (pendingCandleType) {
      currentType = pendingCandleType;
    }

    let nextType: CandleType = defaultType;
    if (currentType && allowedTypes.includes(currentType)) {
      nextType = isTimeLine ? defaultType : currentType;
    }
